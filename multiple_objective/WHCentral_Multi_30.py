...
#tools.cxUniformPartialyMatched, indpb=0.09
...
def main(seed=None):
    ...
    print('Mean cost:', np.min(np.array(save_min_cost)))
    #print(np.shape(np.array(save_min_cost)))
    print('Mean dist:', np.min(np.array(save_min_dist)))
    ...
    np.save('minStats_30.npy', plot_pareto)
    np.save('Hypervolume_30.npy', HyperV_best)
